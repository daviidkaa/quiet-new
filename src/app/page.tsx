      ...
      <div className="min-h-screen bg-black text-white px-4 md:px-8 py-6 overflow-x-hidden">
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((n) => (
                        <button
                          ...
                          className={`w-full text-left p-3 rounded-2xl transition ${n.read ? "hover:bg-zinc-900" : "bg-zinc-900/60 hover:bg-zinc-900"}`}
                        >
        ...
        >
          ...
          <div className="min-w-0">
            <div className="mb-4">
              <h2 className="text-2xl font-medium text-white">Feed</h2>
              <p className="text-zinc-500 text-sm mt-1">
            ...
            {postsLoading ? (
              <div className="space-y-4">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-48 bg-zinc-950 border border-zinc-900 rounded-3xl animate-pulse"
                  />
                ))}
              </div>
            ) : posts.length === 0 ? (
              <div className="bg-zinc-950 border border-zinc-900 rounded-3xl p-10 text-center">
                <div className="text-white font-medium">No posts yet</div>
                <div className="text-zinc-500 text-sm mt-2">
              ...
              <div className="space-y-4">
                {posts.map((p) => {
                  const liked = Boolean(
                    p.likes?.some((l: any) => l.userId === me?.id),
                  );
                  const ownPost = p.userId === me?.id || p.user?.id === me?.id;
                  return (
                    <article
                      key={p.id}
                      className="bg-zinc-950 border border-zinc-900 rounded-3xl p-5 md:p-6"
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="relative">
                          <button
                            ...
                            className="w-9 h-9 rounded-full text-zinc-500 hover:text-white hover:bg-zinc-900 transition"
                          >
                            •••
                          </button>
                          {openMenu === p.id && (
                      ...
                      {p.content && (
                        <div className="mt-4 text-zinc-200 whitespace-pre-wrap leading-7">
                          {p.content}
                        </div>
                      )}
                      {p.image && (
                        ...
                        >
                          <img
                            src={p.image}
                            alt="Post"
                            className="w-full max-h-[650px] object-cover rounded-2xl border border-zinc-900"
                          />
                        </button>
                      )}
                      <div className="flex items-center gap-5 mt-5 text-sm text-zinc-500">
                        <button
                          ...
                          className={`flex items-center gap-2 transition ${liked ? "text-red-400" : "hover:text-white"}`}
                        >
                          <Heart
                            size={17}
          ...
          <div className="hidden xl:block">
            <div className="mb-4">
              <h2 className="text-2xl font-medium text-white">Discover</h2>
              <p className="text-zinc-500 text-sm mt-1">
            ...
            <input
              ...
              className="w-full bg-black/40 border border-zinc-900 rounded-2xl px-4 py-3 outline-none text-white"
            />
            <div className="space-y-3 mt-4">
              {filteredUsers.map((u) => (
                <div
                  key={u.id}
                  className="bg-zinc-950 border border-zinc-900 rounded-2xl p-3 flex items-center justify-between gap-3"
                >
                  <button
                  ...
                  <button
                    ...
                    className={`shrink-0 px-4 py-2 rounded-2xl text-sm transition ${followingUsers[u.id] ? "bg-zinc-900 text-white border border-zinc-800" : "bg-white text-black hover:opacity-90"} ${followingLoading === u.id ? "opacity-60 cursor-wait" : "cursor-pointer"}`}
                  >
        ...
        {showMobilePost && (
          <div className="fixed inset-0 z-[70] bg-black/70 backdrop-blur-sm md:hidden flex items-end pb-[calc(4.5rem+env(safe-area-inset-bottom))]">
            <div className="w-full max-h-[85vh] overflow-y-auto bg-zinc-950 border-t border-zinc-900 rounded-t-3xl p-5">
                ...
                {postImage && (
                  <div className="relative mt-4">
                    <img
                      src={postImage}
                      alt="Post preview"
                      className="w-full max-h-64 object-cover rounded-2xl border border-zinc-900"
                    />
                    <button
                      type="button"
              ...
              <button
                ...
                className="mt-4 w-full bg-white text-black py-3 rounded-2xl font-medium disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Publish
              </button>
            </div>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                {(selectedPost.comments || []).map((c: any) => (
                  <div key={c.id} className="flex gap-3">
                    <div className="w-9 h-9 shrink-0 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-xs">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-zinc-400 mt-1 whitespace-pre-wrap">
                        {c.content}
                      </div>
                    </div>
                  </div>
              ...
              <div className="p-5 border-t border-zinc-900 flex gap-3">
                <input
                  ...
                  className="flex-1 bg-black/40 border border-zinc-900 rounded-2xl px-4 py-3 outline-none text-white"
                />
                <button
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="text-white font-medium">Edit Post</div>
                <button
                  onClick={() => setEditingPost(null)}
                  className="text-zinc-500"
                >
              ...
              <textarea
                ...
                className="w-full min-h-[160px] bg-black/40 border border-zinc-900 rounded-2xl p-4 outline-none text-white resize-none"
              />
              <button
                onClick={saveEdit}
        ...
        <nav className="fixed bottom-0 left-0 right-0 z-[60] md:hidden bg-zinc-950/95 backdrop-blur border-t border-zinc-900 px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
          <div className="max-w-md mx-auto grid grid-cols-4 gap-2">
            <button
              onClick={() => router.push("/")}
              className="h-11 rounded-2xl flex items-center justify-center text-zinc-400 hover:text-white"
            >
              <House size={20} />
            </button>
            <button
